...
const stockInForm = useForm({
    ingredient_id: props.ingredients[0]?.id ?? '', qty: '', note: '',
});
...
const submitIngredient = () => {
    ingredientForm.post(route('owner.inventory.ingredients.store'), {
        preserveScroll: true,
        onSuccess: () => {
            ingredientForm.reset('name', 'current_stock', 'minimum_stock', 'cost_per_unit');
            ingredientForm.unit = 'pcs';
            ingredientForm.is_active = true;
            closeForm();
        },
    });
};
...
const submitStockIn = () => {
    stockInForm.post(route('owner.inventory.stock-in'), {
        preserveScroll: true,
...
const submitStockOut = () => {
    router.post(route('owner.inventory.stock-out'), stockOutForm.data(), {
        preserveScroll: true,
...
const submitAdjust = () => {
    router.post(route('owner.inventory.adjustment'), adjustForm.data(), {
        preserveScroll: true,
        onSuccess: closeModal,
    });
};
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="ingredient in ingredients" :key="ingredient.id" class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button
                                        type="button"
                                        class="mr-1 rounded-lg bg-green-500 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-green-600"
                                        @click="openModal('in', ingredient)"
                                    >+ Masuk</button>
                                    <button
                                        type="button"
                                        class="mr-1 rounded-lg bg-red-500 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-red-600"
            ...
            <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <input v-if="activeModal === 'in'" v-model="stockInForm.qty" type="number" min="0.01" step="0.01" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand" placeholder="0"/>
                        <input v-else-if="activeModal === 'out'" v-model="stockOutForm.qty" type="number" min="0.01" step="0.01" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand" placeholder="0"/>
                        <input v-else v-model="adjustForm.qty" type="number" min="0" step="0.01" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand"/>
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">Catatan (opsional)</label>
                        <input v-if="activeModal === 'in'" v-model="stockInForm.note" type="text" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand" placeholder="Keterangan..."/>
                        <input v-else-if="activeModal === 'out'" v-model="stockOutForm.note" type="text" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand" placeholder="Keterangan..."/>
                        <input v-else v-model="adjustForm.note" type="text" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand" placeholder="Keterangan..."/>
                    </div>
                </div>
