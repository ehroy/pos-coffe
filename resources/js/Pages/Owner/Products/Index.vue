...
const submitVariant = () => {
    ...
    const url = editingVariant.value
        ? route('owner.products.variants.update', [variantModal.value.id, editingVariant.value.id])
        : route('owner.products.variants.store', variantModal.value.id);
...
const deleteVariant = (product, variant) => {
    ...
    router.delete(route('owner.products.variants.destroy', [product.id, variant.id]), { preserveScroll: true });
};
...
const submitCreateProduct = () => {
    ...
    router.post(route('owner.products.store'), fd, {
        preserveScroll: true,
...
const submitEditProduct = () => {
    ...
    router.post(route('owner.products.update', editingProduct.value.id), fd, {
        preserveScroll: true,
    ...
    <AppLayout title="Products Management" :menu-items="menuItems">
        <div v-if="showCreateForm" class="mb-6 rounded-xl border border-brand/30 bg-white p-6 shadow-sm">
            <div class="mb-4 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-slate-800">Add New Product</h3>
                    <p class="text-sm text-gray-500">Create a new menu item for cashier and customer menu.</p>
                </div>
            ...
            <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submitCreateProduct">
                <div>
                    ...
                    <input v-model="createData.name" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2" required />
                ...
                <div>
                    ...
                    <input v-model="createData.base_price" type="number" min="0" step="1" class="w-full rounded-lg border border-gray-300 px-3 py-2" required />
                ...
                <div>
                    ...
                    <select v-model="createData.is_active" class="w-full rounded-lg border border-gray-300 px-3 py-2">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea v-model="createData.description" rows="3" class="w-full rounded-lg border border-gray-300 px-3 py-2"></textarea>
                </div>
                ...
                <div class="md:col-span-2 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="rounded-lg bg-brand px-4 py-2 text-white hover:bg-brand-dark" :disabled="isSubmitting">Save Product</button>
                </div>
            </form>
        </div>
        ...
        <div v-if="showEditForm" class="mb-6 rounded-xl border border-brand/30 bg-white p-6 shadow-sm">
            <div class="mb-4 flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-slate-800">Edit Product</h3>
                    <p class="text-sm text-gray-500">Update product data and replace the image if needed.</p>
                </div>
                <button class="text-sm text-gray-500 hover:text-gray-800" @click="closeEditForm">Close</button>
            </div>

            <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submitEditProduct">
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">Name</label>
                    <input v-model="editData.name" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2" required />
                    <p v-if="errors.name" class="mt-1 text-xs text-red-600">{{ errors.name }}</p>
                </div>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">Base Price</label>
                    <input v-model="editData.base_price" type="number" min="0" step="1" class="w-full rounded-lg border border-gray-300 px-3 py-2" required />
                    <p v-if="errors.base_price" class="mt-1 text-xs text-red-600">{{ errors.base_price }}</p>
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">New Image</label>
                    ...
                    <div class="mt-2 overflow-hidden rounded-lg border border-gray-200">
                        <img :src="editPreviewUrl || editingProduct?.image_url" :alt="editingProduct?.name" class="h-32 w-full object-cover" />
                    </div>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-gray-700">Status</label>
                    <select v-model="editData.is_active" class="w-full rounded-lg border border-gray-300 px-3 py-2">
                        <option :value="true">Active</option>
                        <option :value="false">Inactive</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-gray-700">Description</label>
                    <textarea v-model="editData.description" rows="3" class="w-full rounded-lg border border-gray-300 px-3 py-2"></textarea>
                </div>
                ...
                <div class="md:col-span-2 flex items-center justify-end gap-3">
                    <button type="button" class="rounded-lg border border-gray-300 px-4 py-2 text-gray-700" @click="closeEditForm">Cancel</button>
                    <button type="submit" class="rounded-lg bg-blue-500 px-4 py-2 text-white hover:bg-blue-600" :disabled="isSubmitting">Update Product</button>
                </div>
            </form>
        </div>
                        ...
                        <div class="p-3 sm:p-4">
                            ...
                            <div class="flex gap-2">
                                ...
                                <button
                                    ...
                                    :class="product.is_active ? 'border-red-200 text-red-600 hover:bg-red-50' : 'border-green-200 text-green-600 hover:bg-green-50'"
                                    @click="router.patch(route('owner.products.update', product.id), { is_active: !product.is_active }, { preserveScroll: true })"
                                >
            ...
            <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-hidden">
                ...
                <div class="max-h-64 overflow-y-auto px-6 py-3">
                    <div v-if="variantModal.variants?.length" class="space-y-2">
                        <div v-for="v in variantModal.variants" :key="v.id" class="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5">
                            <div>
                    ...
                    <p v-else class="py-4 text-center text-sm text-gray-400">Belum ada variant.</p>
                </div>
                <div class="border-t border-slate-100 px-6 py-4">
                    <h4 class="mb-3 text-sm font-semibold text-slate-700">{{ editingVariant ? `Edit: ${editingVariant.name}` : 'Tambah Variant Baru' }}</h4>
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-xs font-medium text-gray-600">Nama Variant</label>
                            <input v-model="variantForm.name" type="text" class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-brand" placeholder="Contoh: Large, Hot" />
                    ...
                    <div class="mt-3 flex gap-2">
                        <button type="button" class="flex-1 rounded-xl bg-brand py-2.5 text-sm font-semibold text-white hover:bg-brand-dark disabled:opacity-50" :disabled="variantSubmitting" @click="submitVariant">
                            {{ variantSubmitting ? 'Menyimpan...' : editingVariant ? 'Update' : 'Tambah' }}
                        </button>
                        <button v-if="editingVariant" type="button" class="rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50" @click="editingVariant = null; variantForm = { name: '', price: '', sku: '', barcode: '', is_active: true }">Batal</button>
                    </div>
                </div>
            </div>
        </div>
